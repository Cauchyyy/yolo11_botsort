...
class BOTSORT(BYTETracker):
    ...
    def __init__(self, args, frame_rate=15):
        ...
        self.stable_frames = getattr(args, "stable_frames", 6)
        self.min_votes = getattr(args, "min_votes", 3)
        # perm_id 接管时，要求新轨迹 hits 至少比旧轨迹多 takeover_hits_margin
        self.takeover_hits_margin = getattr(args, "takeover_hits_margin", 2)
        ...
        #  中间线性插值。
        self.fast_speed_px = float(getattr(args, "fast_speed_px", 25.0))
        self.very_fast_speed_px = float(getattr(args, "very_fast_speed_px", 60.0))
            ...
            if very_fast_v > fast_v > 0.0:
                ...
                for tr in tracks:
                    ...
                    if speed <= fast_v:
                        ...
                        a = (very_fast_v - speed) / (very_fast_v - fast_v)
                    alphas.append(a)
                alphas = np.clip(np.asarray(alphas, dtype=float), 0.0, 1.0).reshape(-1, 1)
                # emb_dists' = alpha * emb_dists + (1 - alpha) * 1.0
                emb_dists = alphas * emb_dists + (1.0 - alphas) * 1.0

            # 3) 和原版 BoT-SORT 一样：取 IoU 距离和 ReID 距离的较小值
    ...
    def _apply_cls_soft_penalty(self, tracks, detections, dists):
        """
        S2：类别不一致软惩罚（与速度无关）。
        对于“已经稳定”的轨迹（命中帧数足够 + 投票支持度足够），如果某个检测的
        ReID 类别与该轨迹的 fish_label 不一致，就在代价矩阵上加一个小的 penalty，
        作为“不要串鱼”的软约束。
        """
