...
class BOTSORT(BYTETracker):
    ...
    def __init__(self, args, frame_rate=15):
        ...
        self.cls_mismatch_penalty = float(getattr(args, "cls_mismatch_penalty", 0.04))
    ...
    def _apply_cls_soft_penalty(self, tracks, detections, dists):
        """
        S2 (bonus 版):
        对已经“稳定”的轨迹，只要检测框的 det.cls 与该轨迹的 fish_label 一致，
        就给这一对匹配一个轻微的奖励（减小 dists），作为温和的类别一致性约束。

        注意：不再对 det.cls != fish_label 的 pair 施加惩罚，避免破坏本来正确的匹配。
        """
        ...
        for ti, tr in enumerate(tracks):
            ...
            # 只对“有稳定 fish_label 且投票支持度足够”的轨迹生效
            ...
            for dj, det in enumerate(detections):
                ...
                # ✅ bonus 版：类别一致时减小 dists[ti, dj]，而不是对不一致的 pair 加罚
                if det_cls_int == fish_label:
                    dists[ti, dj] = max(0.0, dists[ti, dj] - penalty)
