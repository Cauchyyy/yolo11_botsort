...
class BYTETracker:
    ...
    def __init__(self, args, frame_rate=30):
        ...
        # ------------------------------------------------------------
        # B3: keep permanent (locked-ID) tracks longer when they are lost
        # If args.track_buffer_perm is not provided, fall back to args.track_buffer
        # ------------------------------------------------------------
        perm_buffer = getattr(args, "track_buffer_perm", None)
        if perm_buffer is None:
            perm_buffer = args.track_buffer
        self.max_time_lost_perm = int(frame_rate / 30.0 * perm_buffer)
    ...
    def update(self, results, img=None):
        ...
        for track in self.lost_stracks:
            ...
            # B3: permanent tracks use a larger lost buffer
            max_lost = self.max_time_lost
            pid = getattr(track, "perm_id", None)
            is_temp = getattr(track, "is_temporary", True)
            if pid is not None and int(pid) > 0 and (not is_temp):
                max_lost = getattr(self, "max_time_lost_perm", self.max_time_lost)
