...
class BOTSORT(BYTETracker):
    ...
    def __init__(self, args, frame_rate=15):
        ...
        # ==== S1/S4：固定 9 条鱼 + 永久 ID 管理 ====
        self.MAX_FISH = 9
        self.used_perm_ids = set()
        self.permanent_tracks = {}
        # 轨迹稳定性判断：至少命中 stable_frames 帧 + 至少 min_votes 次一致投票
        self.stable_frames = getattr(args, "stable_frames", 5)
        self.min_votes = getattr(args, "min_votes", 3)
        # perm_id 接管时，要求新轨迹 hits 至少比旧轨迹多 takeover_hits_margin
        self.takeover_hits_margin = getattr(args, "takeover_hits_margin", 0)

        # ==== S2：类别不一致软惩罚（penalty 版本，而不是 bonus） ====
        # 这个 penalty 是“在代价矩阵上 +penalty”，数值越大表示越不愿意匹配。
        self.cls_mismatch_penalty = float(getattr(args, "cls_mismatch_penalty", 0.07))
        # 只有 ReID 置信度 >= cls_conf_thresh 的 detection 才会可信地参与 S2 约束
        self.cls_conf_thresh = float(getattr(args, "cls_conf_thresh", 0.6))

        # ==== S3：速度自适应地调节 S2 惩罚强度 ====
        # 速度单位：像素/帧，由 BOTrack._update_speed 计算的 bbox 中心位移
        #  v <= fast_speed_px      -> penalty 全额生效（正常游动，强依赖标签）
        #  v >= very_fast_speed_px -> penalty 不再生效（高速/混乱段，少依赖标签）
        #  中间线性插值。
        self.fast_speed_px = float(getattr(args, "fast_speed_px", 20.0))
        self.very_fast_speed_px = float(getattr(args, "very_fast_speed_px", 35.0))
    ...
    def get_dists(self, tracks, detections):
        ...
        if use_reid and len(tracks) > 0 and len(detections) > 0:
            ...
            ##################################################
            # S2: soft label-consistency penalty for stable tracks
    ...
    def _apply_cls_soft_penalty(self, tracks, detections, dists):
        """
        S2 + S3:
          - S2（penalty 版本）：对“稳定轨迹”来说，如果 detection 的 ReID 类别
            与该轨迹的 fish_label 不一致，就在代价矩阵上加一个小的 penalty，
            作为“不要串鱼”的软约束（越大越不愿意匹配）。

          - S3：根据轨迹的运动速度调节 penalty 强度：
              * 鱼慢慢游（v <= fast_speed_px）：speed_factor = 1，penalty 全额起作用；
              * 鱼疯狂乱窜（v >= very_fast_speed_px）：speed_factor = 0，完全不加 penalty；
              * 中间速度：在 [1, 0] 之间线性变化。
        """
        penalty = float(getattr(self, "cls_mismatch_penalty", 0.0))
        if penalty <= 0.0 or dists.size == 0:
            return
        if len(tracks) == 0 or len(detections) == 0:
            return

        fast_v = float(getattr(self, "fast_speed_px", 0.0))
        very_fast_v = float(getattr(self, "very_fast_speed_px", 0.0))
        use_speed = very_fast_v > fast_v > 0.0
        ...
        for ti, tr in enumerate(tracks):
            ...
            # === S3：根据轨迹速度计算 penalty 权重 ===
            if use_speed:
                speed = float(getattr(tr, "speed", 0.0))
                if speed <= fast_v:
                    speed_factor = 0
                elif speed >= very_fast_v:
                    speed_factor = 1
                else:
                    speed_factor = (speed - fast_v) / (very_fast_v - fast_v)
            else:
                # 没设置合理阈值就退化为纯 S2
                speed_factor = 1.0

            eff_penalty = penalty * max(0.0, min(1.0, speed_factor))
            if eff_penalty <= 0.0:
                continue
            ...
            for dj, det in enumerate(detections):
                ...
                # 只有 ReID 置信度足够高才启用 S2/S3 约束
                cls_conf_thresh = float(getattr(self, "cls_conf_thresh", 0.0))
                reid_conf = getattr(det, "reid_conf", None)
                if reid_conf is not None and reid_conf < cls_conf_thresh:
                    continue

                # 惩罚：标签不一致 -> cost += eff_penalty
                if det_cls_int != fish_label:
                    dists[ti, dj] += eff_penalty
