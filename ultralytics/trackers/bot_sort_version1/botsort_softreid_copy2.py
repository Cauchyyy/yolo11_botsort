...
# BoTSORT Soft-ReID variant — Kalman预测 + 匈牙利匹配重写版
# 保留：soft REID、临时轨迹->稳定、tracklet embedding、diagnostics CSV 等逻辑
from typing import List, Optional, Tuple, Callable
import numpy as np
import math, cv2, csv, os, logging
...
# Hungarian assignment: prefer scipy, else simple greedy
try:
    ...
    def hungarian_assign(cost_mat, thresh=float("inf")):
        ...
        while True:
            ...
            if len(used_r)==N or len(used_c)==M: break
...
class Track:
    def __init__(self, bbox: List[float], feat: Optional[np.ndarray], score: float, track_id: int,
                 frame_id: int, kf_obj, tracklet_len=10, vote_history=15, is_temporary=True):
        ...
        if feat is not None and feat.size:
            ...
            self.feat_long = l2_norm(feat).copy()
        else:
            self.feat_long = None
...
class BoTSORTSoftReID:
    ...
    def update(self, results, frame):
        ...
        if len(active_tracks)>0 and len(unmatched_dets_idx)>0:
            ...
            # use hungarian
            cost_copy = cost.copy()
            thresh = 1.0  # reject high-cost assignments
        ...
        currently_matched = set([m[1] for m in matches]) | matched_det | set([d for d in []])
        remaining = [i for i in range(len(dets)) if i not in currently_matched and i not in []]
        if len(remaining)>0 and self.allow_new_tracks:
            for d_idx in remaining:
                tr = Track(dets[d_idx], det_feats[d_idx] if det_feats.size else None, scores[d_idx], self._next_id, self.frame_id, kf_obj=self.kf, tracklet_len=self.tracklet_len, is_temporary=True)
                self._next_id += 1
        ...
        # 10) diagnostics CSV (optional) + assemble outputs
        outs=[]
        if self.diagnostics_csv:
            try:
                with open(self.diagnostics_csv, "a", newline="") as f:
                    ...
                    for di in range(len(dets)):
                        ...
                        if len(self.tracks)>0 and det_feats.size!=0:
                            ...
                            distances = cosine_distance_matrix(act_feats, det_feats[di:di+1]).flatten()
                            idxs = np.argsort(distances)[:3]
                            for ii in idxs:
                                if ii < len(self.tracks):
                                    t_ids.append(self.tracks[ii].track_id)
                                    t_dists.append(float(distances[ii]))
                ...
                logger.debug("diag write fail: %s", e)
